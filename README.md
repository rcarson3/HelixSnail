...
HelixSnail is written with a `no_std` environment being the default environment. I chose this as historically this has appeared the most likely requirement for any GPU or WASM type work that people might do in Rust. So, I wanted to just go with the `no_std` from the start as I really want to eventually run on the GPUs. However, the downside of this is that some of the `const generics` I've tried to use to make things nice still aren't fully backed which forces me to also use nightly features :(. I'd be open to any PRs that might have to fix the nightly requirement if the ergonomics don't suffer too much... Outside of that, I'd also be down for a potential backend that maybe uses `faer` maybe through a feature flag or something? I'll just have to find the time to do that. 
