...
pub(crate) fn lup_decompose<const NDIM: usize, F>(matrix: &mut [F], pivot: &mut [usize], tolerance: F) -> SolverError 
where
    F: Float + Zero + One + NumAssignOps + NumOps + core::fmt::Debug + core::convert::From<f64>,
{
    ...
    for i in 0..NDIM {
        ...
        if max_a < tolerance {
            error!("Pivot too small (pivot: {:?} < tolerance: {:?})", max_a, tolerance);
...
pub(crate) fn lup_solve<const NDIM: usize, F>(solution: &mut [F], matrix: &[F], rhs: &[F], pivot: &[usize])
where
    F: Float + Zero + One + NumAssignOps + NumOps + core::fmt::Debug + core::convert::From<f64>,
{
    assert!(pivot.len() > NDIM + 1);
...
pub(crate) fn lup_solver<const NDIM: usize, F>(matrix: &mut [F], solution: &mut [F], rhs: &[F]) -> SolverError
where
    F: Float + Zero + One + NumAssignOps + NumOps + core::fmt::Debug + core::convert::From<f64>,
    [(); NDIM + 1]:,
{
