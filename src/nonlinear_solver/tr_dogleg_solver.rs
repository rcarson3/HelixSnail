      ...
      [(); NP::NDIM]:,
{
    ...
    fn solve(&mut self) -> NonlinearSolverStatus { NonlinearSolverStatus::Unconverged }
    fn get_num_fcn_evals(&self) -> u32 { 0 }
    fn get_solver_rho(&self) -> F {F::zero()}
    fn get_solver_delta(&self) -> F {F::zero()}
    fn get_l2_error(&self) -> F {F::zero()}
    fn compute_residual_jacobian(fcn_eval: &mut [F], jacobian: &mut [F]) -> bool {false}
}